...
if [[ ! -d "/home/user/training/data/kafka1" ]]; then
  ...
  kafka-storage.sh format -t $KAFKA_CLUSTER_ID -c "$DIR"/config/kafka1.properties
  kafka-storage.sh format -t $KAFKA_CLUSTER_ID -c "$DIR"/config/kafka2.properties
  kafka-storage.sh format -t $KAFKA_CLUSTER_ID -c "$DIR"/config/kafka3.properties
fi
